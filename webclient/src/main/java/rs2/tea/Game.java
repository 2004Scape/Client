...
public final class Game extends GameShell {
	...
	private Buffer login = Buffer.alloc(1);
	...
	private DoublyLinkedList locList = new DoublyLinkedList();
	...
	private DoublyLinkedList temporaryLocs = new DoublyLinkedList();
	...
	private DoublyLinkedList projectiles = new DoublyLinkedList();
	...
	private DoublyLinkedList spawnedLocations = new DoublyLinkedList();
	...
	private DoublyLinkedList spotanims = new DoublyLinkedList();
	...
	private void readZonePacket(@OriginalArg(1) Buffer arg1, @OriginalArg(2) int arg2) {
		@Pc(15) int local15;
		@Pc(24) int local24;
		@Pc(31) int local31;
		@Pc(34) int local34;
			...
			if (arg2 == ZoneProt.OBJ_REVEAL) {
				...
				if (local24 >= 0 && local31 >= 0 && local24 < 104 && local31 < 104) {
					...
					if (this.levelObjStacks[this.currentLevel][local24][local31] == null) {
						this.levelObjStacks[this.currentLevel][local24][local31] = new DoublyLinkedList();
					}
					...
					if (this.showSocialInput) {
						...
						if ((local13 == 13 || local13 == 10) && this.chatTyped.length() > 0) {
							if (this.chatTyped.equals("::clientdrop")) {
								...
								ChatCompression.pack(this.out, this.chatTyped);
								this.out.psize1(this.out.pos - local693);
								this.chatTyped = StringUtils.toSentenceCase(this.chatTyped);
								this.chatTyped = Censor.filter(this.chatTyped);
	...
	private void drawTitleScreen() {
		...
		if (this.titleScreenState == 2) {
			...
			local32 += 15;
			this.fontBold12.drawStringTaggable(local21 / 2 - 88, local32, "Password: " + StringUtils.toAsterisks(this.password) + (this.titleLoginField == 1 & loopCycle % 40 < 20 ? "@yel@|" : ""), true, 16777215);
			local32 += 15;
			local50 = local21 / 2 - 80;
	...
	private boolean handleInterfaceAction(@OriginalArg(1) IfType arg1) {
		...
		if (local4 >= 601 && local4 <= 612) {
			...
			if (this.reportAbuseInput.length() > 0) {
				...
				this.out.p8(StringUtils.toBase37(this.reportAbuseInput));
	...
	private void login(@OriginalArg(0) String arg0, @OriginalArg(1) String arg1, @OriginalArg(2) boolean arg2) {
		...
		try {
			...
			this.out.random = new IsaacRandom(local47);
			...
			if (local237 == 2 || local237 == 18) {
				...
				this.spawnedLocations = new DoublyLinkedList();
				this.friendCount = 0;
				...
				if (this.waveDelay[local46] <= 0) {
					...
					try {
						if (this.waveIds[local46] != this.lastWaveId || this.waveLoops[local46] != this.lastWaveLoops) {
							@Pc(89) Buffer local89 = SoundTrack.generate(this.waveLoops[local46], this.waveIds[local46]);
	...
	private void readPlayerInfo(@OriginalArg(0) Buffer arg0, @OriginalArg(1) int arg1) {
		this.entityRemovalCount = 0;
		this.entityUpdateCount = 0;
	...
	private void drawChatback() {
		...
		if (this.showSocialInput) {
			...
			local135.drawString(4, 90, 0, StringUtils.formatName(this.username) + ":");
		...
		try {
			...
			if (this.packetType == ServerProt.LAST_LOGIN_INFO) {
				...
				if (this.lastAddress != 0 && this.viewportInterfaceId == -1) {
					Signlink.dnslookup(StringUtils.formatIPv4(this.lastAddress));
					this.closeInterfaces();
			...
			if (this.packetType == ServerProt.MESSAGE_PRIVATE) {
				...
				if (!local2652 && this.overrideChat == 0) {
					try {
						...
						@Pc(2721) String local2721 = ChatCompression.unpack(this.in, this.packetSize - 13);
						@Pc(2725) String local2725 = Censor.filter(local2721);
						if (local466 > 1) {
	...
	private void readPlayerUpdates(@OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) Buffer arg3, @OriginalArg(4) PlayerEntity arg4) {
		@Pc(19) int local19;
		if ((arg2 & 0x1) == 1) {
			local19 = arg3.g1();
		...
		if ((arg2 & 0x40) == 64) {
			...
			if (arg4.name != null) {
				...
				if (!local211 && this.overrideChat == 0) {
					try {
						@Pc(244) String local244 = ChatCompression.unpack(arg3, local199);
						@Pc(248) String local248 = Censor.filter(local244);
						arg4.chat = local248;
