...
public final class cache {
	...
	private final jagex2.datastruct.hashtable hashtable = new hashtable(1024);
	...
	public cacheable get(@OriginalArg(0) long arg0) {
		@Pc(5) cacheable local5 = (cacheable) this.hashtable.get(arg0);
		if (local5 != null) {
	...
	public void put(@OriginalArg(1) long arg1, @OriginalArg(2) cacheable arg2) {
		if (this.available == 0) {
			@Pc(8) cacheable local8 = this.history.pop();
			local8.unlink();
			local8.uncache();
		} else {
			this.available--;
	...
	public void clear() {
		while (true) {
			@Pc(3) cacheable local3 = this.history.pop();
			if (local3 == null) {
